<template>
  <div class="h-full flex flex-col glass-card overflow-hidden">
    <div class="p-4 border-b border-white/10">
      ...
      <div class="flex items-center gap-2">
        ...
        <HelpTooltip
          placement="bottom"
          title="搜尋功能"
          description="快速搜尋課程、老師或教室名稱。"
          :usage="['輸入關鍵字即可過濾清單', '支援模糊搜尋']"
        />
      </div>
    </div>
    ...
    <div
      v-if="selectedId"
      class="p-3 border-t border-white/10 flex items-center gap-2"
    >
      <button
        @click="clearSelection"
        class="flex-1 px-4 py-2 rounded-lg bg-slate-700/50 text-slate-300 hover:text-white hover:bg-slate-600 transition-colors text-sm"
      >
        顯示全部課表
      </button>
      <HelpTooltip
        placement="top"
        title="清除篩選"
        description="取消目前的老師或教室篩選，恢復顯示全部排課。"
        :usage="['點擊按鈕即可清除篩選條件']"
      />
    </div>
  </div>
