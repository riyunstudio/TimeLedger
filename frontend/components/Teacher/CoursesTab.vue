          ...
          <div class="flex items-start justify-between mb-3">
            ...
            <div class="flex items-center gap-1">
              <button
                ...
                :class="course.status === 'ACTIVE' ? 'hover:bg-success-500/20 text-success-500' : 'hover:bg-slate-500/20 text-slate-400'"
                :title="course.status === 'ACTIVE' ? '停用' : '啟用'"
              >
...
const toggleCourse = async (course: any) => {
  ...
  const actionText = course.status === 'ACTIVE' ? '停用' : '啟用'

  if (!await alertConfirm(`確定要${actionText}課程「${course.name}」嗎？`)) {
    return
  }

  try {
    const api = useApi()
    await api.patch(`/admin/courses/${course.id}/toggle-active`, { is_active: course.status !== 'ACTIVE' })
