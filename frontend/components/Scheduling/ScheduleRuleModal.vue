    ...
    >
      ...
      <form v-show="!dataLoading && !error" @submit.prevent="handleSubmit" class="p-4 space-y-4">
        ...
        <div class="flex gap-3 pt-2">
          ...
          >
            {{ validationLoading ? '驗證中...' : (loading ? '儲存中...' : (showEditModal ? '儲存修改' : '新增')) }}
          </button>
        </div>
      </form>
    </div>
...
const bufferWarning = ref<string | null>(null)
const bufferWarnings = ref<string[]>([])
 ...
 */
const validateForm = async (): Promise<boolean | 'buffer_warning'> => {
  ...
  // 如果沒有選擇老師或教室，跳過衝突檢查
  if (!form.value.teacher_id && !form.value.room_id) {
    return true
  }
  ...
  try {
    ...
    for (const weekday of form.value.weekdays) {
      ...
      // 如果有硬衝突，直接顯示錯誤
      if (hasHardConflicts(result.hard_conflicts)) {
        conflictError.value = '排課時間與現有規則衝突'
        conflictErrors.value = formatConflictMessages(result.hard_conflicts)
        return false
      }

      // 如果有緩衝衝突，顯示警告
      if (result.buffer_conflicts.length > 0) {
        bufferWarning.value = '緩衝時間不足'
        bufferWarnings.value = formatConflictMessages(result.buffer_conflicts)
  ...
  try {
    ...
    for (const weekday of form.value.weekdays) {
      ...
      if (result.buffer_conflicts.length > 0) {
        ...
        bufferConflictMessages = formatConflictMessages(result.buffer_conflicts)
      }
    }
