...
- **客製化選項**：
  - **主題選擇**：
    ...
    - **Sunset Quartz** (琥珀漸層)
  - **內容元件 (Content)**：
    - **個人品牌資訊**：匯出的圖片上方會顯示 老師頭像、姓名 以及 **3-5 個個人品牌標籤 (Personal Tags)**，增加辨識度。
  - **隱私模式 (Privacy Mode)**：
    - 切換開關：「隱藏行程細節」。
...
- **篩選器**：
  ...
  - **地區篩選 (Region)**：縣市與行政區級聯選單 (Cascading Dropdown)。
  - **子類/Hashtag**：點擊類別後展開的標籤雲 (如：街舞、國標舞) 與熱門 #Hashtag。
  - **關鍵字**：姓名/Bio。
- **結果卡片**：
  - 顯示：頭像、Bio、**服務地區**、**技能分類** (例如 舞蹈 > 街舞)、**技能標籤 ( expertise tags)**。
  - **特色摘要**：下方顯示 3-5 個 **個人品牌標籤 (Personal Branding Tags)**，以不同色調區分。
...
- **Hashtag 輸入組件**：
  - **功能**：標籤輸入與特定的「技能項目」綁定。在選取技能類別或輸入子類後，下方即彈出該技能專屬/推薦的 Hashtag 區域。
  - **模糊搜尋 (Debounced Search)**：輸入時延遲 300ms 呼叫 `/common/hashtags/search` 並彈出建議清單。
  - **動態新增**：若建議清單無相符項，按下 Enter 或逗號後自動將當前輸入轉為新標籤，並記錄於該技能下。
  - **視覺呈現**：以 Chip (標籤塊) 顯示在對應技能卡片內。

- **個人品牌標籤 (Personal Branding Tags)**：
  - 在個人檔案設定頁的最上方或 Bio 旁，提供獨立的標籤編輯區。
  - **限制**：**嚴格限制 3-5 個**。系統會以 `(3/5)` 的即時計數器提示用戶。
  - **用途**：此類標籤不掛載於特定技能，而是作為「老師個人形象」的標籤，主要用於課表圖片匯出的 Header 區域。
