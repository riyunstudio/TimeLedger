...
## 4. 多中心呈現策略 (Multi-Center Visualization)
...
## 5. 多中心配色策略 (Multi-Center Color Strategy)

### 5.1 配色衝突解決方案
當老師同時加入 Center A（紅色）與 Center B（也是紅色）時，採用以下策略：

#### 5.1.1 顏色優先序與亮度調整
- **動態調整**：若兩個中心使用相同色相，系統自動調整亮度 (Lightness) 區分。
- **範例**：
  - Center A: `#FF5733` (標準紅)
  - Center B: `#FF8A65` (淺紅)
  - Center C: `#C70039` (深紅)
...
- 格式：
  ```
  ■ #FF5733 Mozart (鋼琴課、樂理課)
  ■ #8B5CF6 Yoga+ (瑜伽、普拉提)
  ■ #10B981 Dance (街舞、爵士)
  ```
- **交互**：點擊圖例可篩選顯示特定中心的課程（Toggle 開關）。

### 5.2 中心配色管理 (Admin)
- **後台設定**：管理員可在「中心設定」中選擇該中心的代表顏色。
- **預設配色**：新中心建立時，系統從預設配色盤中分配一個尚未使用的顏色。
- **預設配色盤**：
  ```javascript
  const CENTER_COLORS = [
    '#FF5733', // 珊瑚紅
...
- **頂部篩選列**：在課表上方提供中心篩選器。
  ```
  [✓] Mozart  [✓] Yoga+  [✓] Dance  [全部顯示]
  ```
- **預設**：勾選所有已加入的中心。
- **單一中心模式**：點擊某中心可進入「單一中心視圖」，僅顯示該中心的課程。

---

## 6. 循環編輯 UI 流程 (Recurrence Edit UI)

### 6.1 觸發方式
老師長按循環行程中的任一場次，彈出操作選單：
...
1. 彈出確認對話框：
   ...
   └─────────────────────────────────┘
   ```
2. 確認後進入「編輯模式」，僅修改該場次及之後的時間。
3. 系統自動產生：
   - 原規則的 `effective_end` 截斷至前一場
   - 新規則從當前場次開始至原規則結束

#### 6.2.2 編輯「整組循環」
選擇此選項時：
1. 直接進入編輯表單，可修改循環規則（`interval`、`weekdays`、`until`）。
2. 所有場次（含過去與未來）皆受影響。
...
1. 顯示受影響場次預覽：
   ...
   └─────────────────────────────────┘
   ```
2. 刪除後，該循環規則的 `effective_end` 截斷至前一場。
...
| 狀態 | 視覺呈現 |
|:---|:---|
| 原循環規則（未修改） | 實線邊框，無特殊標記 |
| 編輯後的單一場次（例外） | **虛線邊框**，右上角顯示 `⚠️` 標記 |
| 刪除後的場次 | 灰色背景，顯示刪除線 |
| 審核中的編輯申請 | 斜線陰影，顯示 `⏳` icon |

### 6.5 API 對應
| UI 行為 | API 參數 |
|:---|:---|
| 編輯這場 | `update_mode: "SINGLE"` + `start_at`/`end_at` 變更 |
| 編輯這場及之後所有 | `update_mode: "FUTURE"` + `start_at`/`end_at` 變更 |
| 編輯整組循環 | `update_mode: "ALL"` + `recurrence` 變更 |
| 刪除這場 | `DELETE` + `mode: "SINGLE"` |
| 刪除這場及之後所有 | `DELETE` + `mode: "FUTURE"` |
| 刪除整組循環 | `DELETE` + `mode: "ALL"` |
