...
2. **Frontend** 準備一個 **Hidden Container (`position: absolute; left: -9999px`)**。
   ...
   - 在容器內 Render 一個「專用匯出 Component」：
     - Header: LOGO、老師姓名、日期範圍 (2026/01/19 ~ 01/25)。
...
3. **Buffer Check (緩衝)**:
   ...
   - **Room Buffer**: `New.Start - Prev.End < center.room_buffer` ?
   - **Teacher Buffer**: 若已指派老師，則驗證 `New.Start - Prev.End < center.teacher_buffer` ? (若無老師則跳過)。
   - **結果**：若 True，報錯 `E_BUFFER`。
     - 檢查 `policy.allow_override`。若 False → 阻擋。
     - 若 True → 標記 `conflict`，允許帶 `override=true` 旗標送出 (可能進入 Pending)。
...
當老師儲存個人檔案時：
1. **解析輸入**：前端傳入字串陣列 (e.g. `["#街舞", "#新標籤"]`)。
2. **Upsert Dictionary**：
   - 後端檢查 `hashtags` 表中是否已存在該標籤。
   - 若不存在：新增至 `hashtags` 表。
   - 若存在：不動作。
3. **更新關聯**：
   - 更新 `teacher_hashtags` 關聯表。
   - **更新 `usage_count`**：重新計算並更新該標籤在 `hashtags` 表中的使用次數。

### 5.2 自動清理 (Garbage Collection)
為了避免無人使用的錯誤標籤堆積：
1. **同步清理**：在更新 `usage_count` 後，若偵測到某標籤的 `usage_count == 0`，則立即從 `hashtags` 表中刪除。
2. **排程檢查 (選配)**：每日凌晨掃描一次 `usage_count = 0` 的殘留資料並剔除。
### 5.3 輸入行為 UX 規範 (Behavioral UX)
