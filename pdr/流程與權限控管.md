...
sequenceDiagram
    ...
    Validate_Engine->>DB: Check Overlap (Teacher/Room) & Buffer & Capacity
...
sequenceDiagram
    ...
    Admin->>Talent_Search_UI: 輸入 City/District / Category / Hashtags
    Talent_Search_UI->>Backend: GET /admin/talent/search?city=...&district=...
    Backend->>DB: Query Teachers (is_open_to_hiring=true)
    DB-->>Backend: List of Candidates
    Backend-->>Talent_Search_UI: {Name, Bio, Skills, Hashtags, City, District}
...
| **Room Management** | CRUD (Add/Remove Rooms) | ❌ | ✅ | 包含設定容納人數 |
