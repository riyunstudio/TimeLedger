...
func (r *CreateRuleRequest) Validate() error {
	// 驗證 Status
	if r.Status == "" {
		r.Status = models.RuleStatusConfirmed // 預設值
	} else if !models.IsValidRuleStatus(r.Status) {
		return fmt.Errorf("invalid status: %s, valid values are: %v", r.Status, models.ValidRuleStatuses)
	}
	return nil
}
...
func (r *UpdateRuleRequest) Validate() error {
	// 驗證 Status
	if r.Status != "" && !models.IsValidRuleStatus(r.Status) {
		return fmt.Errorf("invalid status: %s, valid values are: %v", r.Status, models.ValidRuleStatuses)
	}
	return nil
}
