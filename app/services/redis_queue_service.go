...
func (s *RedisQueueService) logInfo(msg string, keysAndValues ...interface{}) {
	if s.log != nil {
...
func (s *RedisQueueService) logWarn(msg string, keysAndValues ...interface{}) {
	if s.log != nil {
...
func (s *RedisQueueService) logError(msg string, keysAndValues ...interface{}) {
	if s.log != nil {
...
func (s *RedisQueueService) PushToRetry(ctx context.Context, item *NotificationItem) error {
	...
	if item.RetryCount >= MaxRetryCount {
		...
		s.logWarn("Notification exceeded max retries, discarded",
	...
	s.logInfo("Notification queued for retry",
		"notification_id", item.ID,
		"type", item.Type,
		"retry_count", item.RetryCount,
	)
